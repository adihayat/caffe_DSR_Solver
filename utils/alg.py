...
'''
def curve(X,Y):
    return 0.1*X**2 + 0.1*np.cos(Y) + 0.1*(Y**2)*(Y>0)

def curve_grad(X,Y):
    return np.array((0.2*X + np.random.normal(0,GlobalNoisePower,X.shape),-0.1*np.sin(Y) + 0.2*Y*(Y>0) + np.random.normal(0,GlobalNoisePower,Y.shape)))
